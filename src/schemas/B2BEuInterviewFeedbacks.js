...
// --- Begin: B2B Partner End Users Interview Feedbacks Schema --- //
const schema = new Schema({
  ...
  intrw: {type: String, required: true}, // Interview Record ID: _id
  intrwId: {type: String, required: true}, // Interview ID: AS1289I61577(First Name, Last Name, Current Year(2024) - 2023, Day of Year(289 - 15th Oct), 'I' for Interview, Current time in seconds(17:06:17))
  euUser: {type: String, required: true},
  euName: {type: String, required: true},
  submission: {type: String, required: true}, // Submission Record ID: _id
  ...
  canName: {type: String, required: false}, // Candidate Name
  canPhNum: {type: String, required: false}, // Candidate Phone Number
  jobTitle: {type: String, required: true},
  jobId: {type: String, required: false},
  jobDesc: {type: String, required: false},
  skills: {type: String, required: true},

  intrwStatus: {type: String, required: false}, // Next Round, Selected, On Hold, Not Selected, Other
  intrwTyEmail: {type: Boolean, required: false}, // Thank you email for interviewer?
  intrwNotes: {type: String, required: false}, // Interview Notes
  intrwdBy: {type: String, required: false}, // Name of the persons who interviewed you?

  testScore: {type: Number, required: false},
  testStatus: {type: String, required: false}, // Passed, Failed
  testNotify: {type: Boolean, required: false}, // Notifying the vendor about the test being completed?
  testNotes: {type: String, required: false}, // Test result notes

  qus: {type: String, required: true}, // Questions
  ifNotes: {type: String, required: false}, // Interview Feedback Notes
  panelMembers: {type: String, required: false},
  testCompany: {type: String, required: false}, // Test Company Name
  ifPath: {type: String, required: false}, // Interview Image Path
...
schema.index({delFlag: -1, b2b: 1, intrwId: 1});
schema.index({delFlag: -1, b2b: 1, submission: 1});
schema.index({delFlag: -1, b2b: 1, subId: 1});
schema.index({cDtStr: -1, uDtStr: -1});

module.exports = mongoose.model(config.collB2BEuInterviewFeedbacks, schema);
// --- End: B2B Partner End Users Interview Feedbacks Schema --- //
